...
object Day10 : Day {
    ...
    private fun canObserve(observer: Point, observable: Point): Boolean {
    ...
    private fun asteroidsInBetween(observer: Point, observable: Point): Int {
        ...
        for (i in 1 until steps) {
            val pointInBetween = Point(stepX * i, stepY * i).plus(observer)
    ...
    override fun part2(): Any {
        ...
        val candidates = mapWithoutStation.filter { asteroidsInBetween(it, station) == 0 }.sortedWith ( Comparator { p1, p2 -> compare(p1, p2) })
        val asteroid200 = candidates[199]
        return asteroid200.x * 100 + asteroid200.y
    }

    private fun compare(p1: Point, p2: Point) : Int {
        val v1 = Point(p1.x - station.x, station.y - p1.y)
        val v2 = Point(p2.x - station.x, station.y - p2.y)
        val q1 = quadrant(v1)
        val q2 = quadrant(v2)
        if(q1 != q2) {
    ...
    private fun angle(v1: Point): Double {
        return atan2(v1.x.toDouble(), v1.y.toDouble())
    }

    private fun quadrant(it: Point): Int {
        if (it.x >= 0) {
            if (it.y >= 0) {
                return 1
            }
    ...
    private fun toPoints(mapLines: List<String>): List<Point> {
